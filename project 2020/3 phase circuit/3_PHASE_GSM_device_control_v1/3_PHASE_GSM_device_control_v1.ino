...
void setup() {
  ...
  initialize();
  RecieveMessage();
  Serial.println("AT+CMGD=1,4");    //Sets the GSM Module in Text Mode
...
void operate_device()
{
  while (Serial.available() > 0)
  {
    ...
    if (Secrete_char != -1)
    {
      ...
      lcd.print(val_op);
      lcd.setCursor(0, 1 );
      lcd.print(data.substring(Secrete_char));
      delay(2000);
      switch (val_op)
      {
        case '1': {
...
void RecieveMessage()
{
  ...
  Serial.println("AT+CMGF=1"); // AT Command to recieve a live
  delay(1000);
...
void sendsms(int state)
{
  ...
  if (state == 1)  {
    ...
    else if (Phase2Phase_Fault == 1) Serial.println("PHASE TO PHASE FAULT OCCURED");
    else if (Earth_Fault == 1) Serial.println("EARTH FAULT");
    else Serial.println("ALL FAULT clear \n System OK ... "); // The SMS text you want to send
  }
  ...
  if (state == 3)
  {
    ...
    if (Phase_fault == 1)
    {
      if (R_phase_val < 300 )   //if (R_phase_val < 300 || B_phase_val < 300 || Y_phase_val < 300)
        Serial.print("R ");

      if (B_phase_val < 300 )   //if (R_phase_val < 300 || B_phase_val < 300 || Y_phase_val < 300)
        Serial.print("B ");

      if (Y_phase_val < 300 )   //if (R_phase_val < 300 || B_phase_val < 300 || Y_phase_val < 300)
        Serial.print("Y ");

      Serial.println("PHASE NOT AVAILABLE FAULT");
    }
...
void readSMS()
{
  Serial.println("AT+CMGR=1");    //Sets the GSM Module in Text Mode
